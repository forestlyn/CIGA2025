...
namespace CardHouse
{
    public class CurrencyRegistry : MonoBehaviour
    {
        ...
        void ShowPlayerWallet(int playerIndex)
        {
            ...
            Debug.Log($"Showing wallet for player {playerIndex} with {PlayerWallets[playerIndex].Currencies.Count} wallets available.");
            foreach (var currency in PlayerWallets[playerIndex].Currencies)
            {
                Debug.Log($"Adding currency {currency.Amount} to display.");
        ...
        public void AdjustCurrency(string name, int playerIndex, int amount)
        {
            ...
            if (currency != null)
            {
                ...
                //Debug.Log($"Adjusted currency '{name}' for player {playerIndex} by {amount}. New amount: {currency.Amount}");
            }
            else
            {
                Debug.LogWarning($"Currency '{name}' not found for player {playerIndex}.");
            }
        }
