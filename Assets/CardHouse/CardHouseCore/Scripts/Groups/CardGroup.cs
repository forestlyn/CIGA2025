...
namespace CardHouse
{
    ...
    public class CardGroup : MonoBehaviour
    {
            ...
            if (cardComponent != null && cardDragHandler != null)
            {
                ...
                switch (cardComponent.GetComponent<DragOperator>().DragAction)
                {
                    ...
                    case DragAction.UseAndDiscard:
                        ...
                        {
                            Debug.LogWarningFormat("{0}: Could not find Discard group to discard this CardId", name);
                            cardComponent.Group?.ApplyStrategy();
                            break;
                        }
        ...
        public void Mount(Card card, int? index = null, bool instaFlip = false, SeekerSetList seekerSets = null, SeekerSet seekersForUnmounting = null, bool invokeEvent = true)
        {
            if (card == null)
            {
                Debug.LogWarning("Cannot mount a null CardId");
                return;
            }
        ...
        { // returns index of CardId if found, -1 if not found
            int? index = null;
